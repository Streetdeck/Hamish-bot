...
config_file = open('./siteconfig.json', 'w+', encoding='utf-8').read()
...
for task in config_data:
    ...
    repo_info_str = urllib.request.urlopen(url).read().decode("utf8")
    try:
    ...
    for file_path in commit_info['files']:
        ...
        record_page.save(summary=summary, asynchronous=True,  minor=False)
